...
- Some answers may become clearer once you’ve looked ahead at the code later in the script. Consider revisiting questions as you go.
:::
...
8. **What is the purpose of rescaling `opacity_val`?**
...
10. **In your own words, what is the difference between `paste0()` and `glue()`? Why did the author use `paste0` to construct `quotes` and `glue` to construct the other text elements?**
...
**HINT:** Consider temporarily commenting out and/or rearranging the `geom_*()` layers to better understand how this plot is constructed
...
ggsave(plot = plot_final, 
       filename = here::here("outputs", "test.png"), 
       height = 16, 
       width = 10)
```
...
5. **How has AI assisted you (if at all) in interpreting this code? In what ways was looking at the documentation more (or potentially less) helpful?**
